...
`CoolNether123.FilterSignals`, supported version 1.6, and declared Harmony
...
Final lane `FilterSignals-c16476171867471a97186bbf5a507909`
verified:
...
- save/load, post-load UI and filter-state checks, and loading the resulting
  save without FilterSignals to confirm safe removal;
- final log review with zero FilterSignals exceptions, errors, repeated warnings,
  or Harmony failures.
...
without FilterSignals and without error or exception matches. All lanes exited
...
Automated coverage now reports `PASS: 17 FilterSignals domain tests`. The added
...
`RWT-BUILD-PACKAGE-VALID`; the packaged `FilterSignals.dll` is 65,536 bytes
with SHA-256
`52014D7DAE3F29EF7281422F9386C6C7C430448618278B00929184F1ACA5A996`.

Isolated lane `FilterSignals-e971922cf9c74a2f9d8b3fa945c2b082`
used exactly Core, Harmony, RimWorld Agent, Spine, and Filter Signals:
...
- The fixture continued to display `Permanent filter state: unchanged`.
  Log review found no exception matches or FilterSignals navigation errors.
- The lane stopped with exit code 0 and `ForcedTermination=false`.

## Camera-preserving navigation polish

Final lane:
`FilterSignals-06b03f88bbc14606b4ea060880b57cbc`
...
- Hover text contains only a short status, short reason, and action line when
  ...
  verification below.
- Capture `filter-signals-architect-camera-preserved-final-20260731-055936-651.png`
  records the open Architect tab, active Kibble row, and short three-line
  FilterSignals text without changing the visible map position.
...
- The live log contains zero UI-root, illegal-OnGUI, FilterSignals navigation, or
...
Final lane:
`FilterSignals-f5b1df97e54e40689884662915ae0541`
...
- The 17 existing domain tests passed unchanged. The toolbar-default,
  ...
  passed.
- Capture `filter-signals-square-single-tooltip-20260731-214617-318.png` shows that
  hovering Kibble's colored square displays only the short three-line
  FilterSignals tooltip. RimWorld's item description is no longer stacked with it.
- Capture `filter-signals-row-vanilla-tooltip-preserved-20260731-214637-649.png`
...
- The shipping DLL is 66,048 bytes with SHA-256
  ...
  Package validation returned `RWT-BUILD-PACKAGE-VALID`.
- The live log contains zero UI-root, illegal-OnGUI, FilterSignals error, root
...
Final lane:
`FilterSignals-6bf2ce502b794c12b075783c8b419fa5`
...
- Capture `filter-signals-small-volume-expanded-20260731-233706-808.png` shows gold
...
Final lane:
`FilterSignals-3f82afc247724eaa9df4b932ce3c444c`
...
- Clicking Chocolate's non-actionable square left the filter fixture visibly
  open. Capture `filter-signals-inert-after-click-storage-open-20260731-232240-024.png`
...
The status square now registers its sole FilterSignals tooltip directly after
...
twice by FilterSignals and Spine.
